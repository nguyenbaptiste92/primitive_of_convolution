...
#include "layers/nnom_groupconv2d.h"
...
{
	nnom_groupconv2d_layer_t *layer;
	nnom_buf_t *comp;
	nnom_layer_io_t *in, *out;
	size_t mem_size;

	// allocate a block memory for all the sub handles and shifts.
	mem_size = sizeof(nnom_groupconv2d_layer_t) + sizeof(nnom_layer_io_t) * 2 + sizeof(nnom_buf_t);
	layer = nnom_mem(mem_size);
	if (layer == NULL)
		return NULL;
	
	// distribut the memory to sub handles.
	in = (void *)((uint8_t*)layer + sizeof(nnom_groupconv2d_layer_t));
	...
	layer->super.free = groupconv2d_free;
  ...
  layer->groups = config->groups; // number of group
					 ...
					 const nnom_weight_t *w, const nnom_bias_t *b)
{
	...
	size_t mem_size = sizeof(nnom_groupconv2d_layer_t) + sizeof(nnom_layer_io_t) * 2 + sizeof(nnom_buf_t);
	layer = nnom_mem(mem_size);
	if (layer == NULL)
		return NULL;
...
nnom_status_t groupconv2d_run(nnom_layer_t *layer)
{
	...
	if(layer->in->tensor->bitwidth == 8 && cl->weight->qtype == NNOM_QTYPE_PER_TENSOR)
	{
		...
		if ((layer->in->tensor->dim[2] % 4 == 0) && (layer->out->tensor->dim[2] % 2 == 0))
		{
			...
			status = (nnom_status_t)arm_group_convolve_HWC_q7_fast_nonsquare(
	...
	#ifdef USE_IM2COL
	local_groupconvolve_HWC_q7_nonsquare_im2col(
				layer->in->tensor->p_data,
				layer->in->tensor->dim[1], layer->in->tensor->dim[0], layer->in->tensor->dim[2],
				cl->weight->p_data, layer->out->tensor->dim[2], cl->groups,
				cl->kernel.w, cl->kernel.h, cl->pad.w, cl->pad.h, cl->stride.w, cl->stride.h,
