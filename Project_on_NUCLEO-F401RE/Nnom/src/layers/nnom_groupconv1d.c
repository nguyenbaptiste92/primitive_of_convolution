...
#include "nnom.h"
#include "nnom_local_groupconv1d.h"
#include "nnom_layers.h"
#include "layers/nnom_groupconv1d.h"
...
nnom_layer_t *GroupConv1D(uint32_t filters, uint32_t groups, nnom_3d_shape_t k, nnom_3d_shape_t s, nnom_3d_shape_t d,  nnom_padding_t pad_type,
					 const nnom_weight_t *w, const nnom_bias_t *b)
{
	nnom_groupconv1d_layer_t *layer;
	nnom_buf_t *comp;
	nnom_layer_io_t *in, *out;
	// apply a block memory for all the sub handles.
	size_t mem_size = sizeof(nnom_groupconv1d_layer_t) + sizeof(nnom_layer_io_t) * 2 + sizeof(nnom_buf_t);
	layer = nnom_mem(mem_size);
	if (layer == NULL)
		return NULL;

	// distribut the memory to sub handles.
	in = (void *)((uint8_t*)layer + sizeof(nnom_groupconv1d_layer_t));
	out = (void *)((uint8_t*)in + sizeof(nnom_layer_io_t));
	comp = (void *)((uint8_t*)out + sizeof(nnom_layer_io_t));

	// set type in layer parent
	layer->super.type = NNOM_GROUPCONV_1D;
	// set buf state
	...
	layer->super.run = groupconv1d_run;
	layer->super.build = groupconv1d_build;
...
{
	nnom_groupconv1d_layer_t *cl = (nnom_groupconv1d_layer_t *)layer;

	// get the tensor from last layer's output
	layer->in->tensor = layer->in->hook.io->tensor;

	// create new tensor for the output
	layer->out->tensor = new_tensor(NNOM_QTYPE_PER_TENSOR, layer->in->tensor->num_dim, cl->filters);
	...
	layer->out->tensor->dim[1] = groupconv1d_output_length(layer->in->tensor->dim[1], cl->kernel.w, cl->padding_type, cl->stride.w, cl->dilation.w);
	layer->out->tensor->dim[2] = cl->filters; // channel stays the same
...
nnom_status_t groupconv1d_run(nnom_layer_t *layer)
{
	...
	if(layer->in->tensor->bitwidth == 8 && cl->weight->qtype == NNOM_QTYPE_PER_TENSOR)
	{
		...
		if ((layer->in->tensor->dim[2] % 4 == 0) && (layer->out->tensor->dim[2] % 2 == 0))
		{
			...
			status = (nnom_status_t)arm_group_convolve1D_HWC_q7_fast(
