...
#include "layers/nnom_addconv2d.h"
...
{
	nnom_addconv2d_layer_t *layer;
	nnom_buf_t *comp;
	nnom_layer_io_t *in, *out;
	size_t mem_size;

	// allocate a block memory for all the sub handles and shifts.
	mem_size = sizeof(nnom_addconv2d_layer_t) + sizeof(nnom_layer_io_t) * 2 + sizeof(nnom_buf_t);
	layer = nnom_mem(mem_size);
	if (layer == NULL)
		return NULL;
	
	// distribut the memory to sub handles.
	in = (void *)((uint8_t*)layer + sizeof(nnom_addconv2d_layer_t));
	out = (void *)((uint8_t*)in + sizeof(nnom_layer_io_t));
	comp = (void *)((uint8_t*)out + sizeof(nnom_layer_io_t));

	// set type in layer parent
	layer->super.type = NNOM_ADDCONV_2D;
	// set buf state
	...
	layer->super.run = addconv2d_run;
	layer->super.build = addconv2d_build;
	layer->super.free = addconv2d_free;
					 ...
					 const nnom_weight_t *w, const nnom_bias_t *b, const nnom_addconv_parameter_t *add_param)
{
	...
	size_t mem_size = sizeof(nnom_addconv2d_layer_t) + sizeof(nnom_layer_io_t) * 2 + sizeof(nnom_buf_t);
	layer = nnom_mem(mem_size);
	if (layer == NULL)
		return NULL;
...
{
	nnom_addconv2d_layer_t *cl = (nnom_addconv2d_layer_t *)layer;
  ...
  if (cl->mode==2){
      layer->out->tensor->q_dec[0] = layer->in->tensor->q_dec[0] - cl->output_rshift[0]; // need some modification for 16bit.
  }
  else{
      layer->out->tensor->q_dec[0] = cl->weight->q_dec[0] - cl->output_rshift[0]; // need some modification for 16bit.
  }
	...
	layer->out->tensor->dim[0] = addconv_output_length(layer->in->tensor->dim[0], cl->kernel.h, cl->padding_type, cl->stride.h, cl->dilation.h);
	layer->out->tensor->dim[1] = addconv_output_length(layer->in->tensor->dim[1], cl->kernel.w, cl->padding_type, cl->stride.w, cl->dilation.w);
...
nnom_status_t addconv2d_run(nnom_layer_t *layer)
{
  ...
  // Need HWC format
  #ifdef USE_IM2COL
	local_addconvolve_HWC_q7_nonsquare_im2col(
					...
					cl->bias->p_data, cl->bias_lshift[0], cl->output_rshift[0],cl->mode,cl->inter_lshift[0],
					layer->out->tensor->p_data,
					layer->out->tensor->dim[1], layer->out->tensor->dim[0], (q7_t *)(layer->comp->mem->blk), NULL);

	#else
	if (cl->mode==0){
	      local_add_convolve_HWC_q7_nonsquare_0(
					layer->in->tensor->p_data,
					layer->in->tensor->dim[1], layer->in->tensor->dim[0], layer->in->tensor->dim[2],
					cl->weight->p_data, layer->out->tensor->dim[2],
					cl->kernel.w, cl->kernel.h, cl->pad.w, cl->pad.h, cl->stride.w, cl->stride.h, cl->dilation.w, cl->dilation.h,
					cl->bias->p_data, cl->bias_lshift, cl->output_rshift,cl->inter_lshift, cl->weight->qtype,
					layer->out->tensor->p_data,
					layer->out->tensor->dim[1], layer->out->tensor->dim[0], NULL, NULL);
	  }
	  else if (cl->mode==1){
	      local_add_convolve_HWC_q7_nonsquare_1(
					layer->in->tensor->p_data,
					layer->in->tensor->dim[1], layer->in->tensor->dim[0], layer->in->tensor->dim[2],
					cl->weight->p_data, layer->out->tensor->dim[2],
					cl->kernel.w, cl->kernel.h, cl->pad.w, cl->pad.h, cl->stride.w, cl->stride.h, cl->dilation.w, cl->dilation.h,
	      ...
	      local_add_convolve_HWC_q7_nonsquare_2(
					layer->in->tensor->p_data,
					layer->in->tensor->dim[1], layer->in->tensor->dim[0], layer->in->tensor->dim[2],
					cl->weight->p_data, layer->out->tensor->dim[2],
					cl->kernel.w, cl->kernel.h, cl->pad.w, cl->pad.h, cl->stride.w, cl->stride.h, cl->dilation.w, cl->dilation.h,
