...
q7_t* local_mat_mult_kernel_for_conv(const q7_t *pA,
...
int local_convolve_HWC_q7_nonsquare_im2col(const q7_t *Im_in,
                                              ...
                                              q7_t *bufferB)
{
    ...
    for (i_out_y = 0; i_out_y < padding_y; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++)
        {
            ...
            {
                pOut = local_mat_mult_kernel_for_conv(
                    wt, bufferA, ch_im_out, sizepatch, bias_shift, out_shift, bias, pOut);
                /* counter reset */
                pBuffer = bufferA;
            }
        }
...
q7_t* local_mat_mult_kernel_for_depthwiseconv(const q7_t *pA,
                                              const q7_t *pInBuffer,
                                              const uint16_t ch_im_out,
                                              const uint16_t numCol_A,
                                              const uint16_t bias_shift,
                                              ...
                                              q7_t *pOut)
{
    ...
    q7_t *pOut2 = pOut + ch_im_out;
    int i;
    int patch_size = numCol_A * ch_im_out;
    ...
    /* this loop over rows in A */
    for (i = 0; i < ch_im_out; i += 1)
    {
        const q7_t *pB = pInBuffer + i;
        const q7_t *pB2 = pB + patch_size;
...
int local_depthwise_separable_conv_HWC_q7_nonsquare_im2col(const q7_t *Im_in,
                                                         const uint16_t dim_im_in_x,
                                                         const uint16_t dim_im_in_y,
                                                         const uint16_t ch_im_in,
                                                         const q7_t *wt,
                                                         ...
                                                         q7_t *bufferB)
{
    ...
    for (i_out_y = 0; i_out_y < dim_im_out_y; i_out_y++)
    {
        for (i_out_x = 0; i_out_x < dim_im_out_x; i_out_x++)
        {
            ...
            if (pBuffer == bufferA + size2patches)
            {
                pOut = local_mat_mult_kernel_for_depthwiseconv(wt, bufferA, ch_im_out, dim_kernel_x*dim_kernel_y, bias_shift, out_shift, bias, pOut);
                /* counter reset */
                pBuffer = bufferA;
            }
        }
