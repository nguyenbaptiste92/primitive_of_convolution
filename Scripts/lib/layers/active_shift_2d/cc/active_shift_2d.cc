...
REGISTER_OP("Active_Shift_2D_Backprop")
.Input("x: T")		// [ batch, in_channels, in_rows, in_cols]
.Input("shift: T")	// [ 2, in_channels]
.Input("out_grad: T")
.Output("x_grad: T")
...
#define REGISTER_GPU(T)                                                \
  REGISTER_KERNEL_BUILDER(                                          \
      Name("Active_Shift_2D").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
      ActiveShift2DOp<GPUDevice, T>);                                    \
  REGISTER_KERNEL_BUILDER(                                          \
      Name("Active_Shift_2D_Backprop").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
